<template>
  <div class="article_index_container">
    <Timeline pending>
      <TimelineItem v-for="(postYear, idx) in Object.keys(postYears).reverse()"
                    :key="postYear">
        ...
        <div class="content"
             v-for="(article, index) in postYears[postYear]"
             :key="index">
          <div class="content_inner">
            <p class="article_title"
               :data-uuid="article.uuid.replace(/^([a-zA-Z0-9]*).*/, '$1')"
               @click="gotoArticleDetail">-【{{article.title}}】</p>
            <!--<p class="post_time">{{article.tag}}</p>-->
            <div class="member_level_tag">
              <p class="member_level_tag_item"
                 :style="{backgroundColor: theme}"
                 v-for="(t, i) in filterTags(article.tag).split(';')"
                 :key="i"
                 v-text="t"></p>
              <Tooltip content="修改标签"
                       placement="right">
                <div class="modify_tag_icon"
                     :data-post-year="postYear"
                     :data-index="index"
                     :data-album="article.album"
                     :data-title="article.title"
                     :data-tags="article.tag"
                     :data-uuid="article.uuid.replace(/^([a-zA-Z0-9]*).*/, '$1')"
                     @click="showModifyTagModal">
                  <Icon type="ios-cog"
                        size="24"
      ...
      <TimelineItem>
        <a href="javascript:void(0)"
           @click="loadNextPage"
           v-if="totalPages > pageIndex">查看更多</a>
        <span class="loaded_text"
              v-else>已经加载完毕</span>
      </TimelineItem>
    </Timeline>
    ...
    <div class="operation_container">
      <Tooltip content="新建文章"
        ...
        <div class="operation_item"
             @click="showCreateNewArticleModal">
    ...
    <Modal v-model="newArticle.shown"
           title="新建文章"
           :loading="newArticle.loading"
           @on-ok="createNewArticle">
      <Form :model="newArticle"
            :label-width="40">
        <FormItem label="封面">
          <div class="upload_item">
            <upload-image width="200"
                          ...
                          @success="setArticleAlbum"
                          @fail="uploadFailed">
              <img class="article_album"
                   style="opacity: 1!important;"
                   :src="newArticle.album || assets.articleAlbum"
                   alt="封面">
            </upload-image>
            <div class="progress_item"
                 v-if="uploadProgress.shown">
              <i-circle :percent="uploadProgress.percent">
                <span class="demo-Circle-inner"
                      style="font-size:24px">{{uploadProgress.percent}}%</span>
              </i-circle>
            </div>
          </div>
        </FormItem>
        <FormItem label="标题">
          <Input v-model="newArticle.title"
                 placeholder="给文章起个标题先"
                 style="width: 100%;" />
        </FormItem>
        <FormItem label="标签">
          <Select v-model="newArticle.tag"
                  filterable
                  multiple>
            <OptionGroup v-for="(g, index) in allTagsForAdd"
                         :key="g.value"
                         :label="g.text">
              <Option v-for="(t, idx) in g.children"
                      :key="t.value"
                      :value="t.value"
                      :label="t.text">
                <span v-text="t.text"
                      :title="t.message"></span>
                <span style="float: right; color: #cccccc;"
                      v-text="t.message"></span>
              </Option>
            </OptionGroup>
          </Select>
        </FormItem>
    ...
    <Modal v-model="modifyTag.shown"
           title="修改文章标签"
           :loading="modifyTag.loading"
           @on-ok="confirmModifyTag">
      <Form :model="modifyTag"
            :label-width="40">
        <FormItem label="封面">
          <div class="upload_item">
            <upload-image width="200"
                          ...
                          @before-upload="beforeUpload"
                          @progress="uploadImageProgress"
                          @success="setArticleAlbum2"
                          @fail="uploadFailed">
              <img class="article_album"
                   style="opacity: 1!important;"
                   :src="modifyTag.album || assets.articleAlbum"
                   alt="封面">
            </upload-image>
            <div class="progress_item"
                 v-if="uploadProgress.shown">
        ...
        <FormItem label="标题">
          <input v-model="modifyTag.title"
                 disabled
                 class="ban_input" />
        </FormItem>
        <FormItem label="标签">
          <Select v-model="modifyTag.tag"
                  filterable
                  multiple>
            <OptionGroup v-for="(g, index) in allTagsForAdd"
                         :key="g.value"
