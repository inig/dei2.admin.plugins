<template>
  <div class="property_activity_container">
    <Form class="property_form"
          :label-width="60">
      <FormItem label="页面动画">
        <Select v-model="activeComponent.template.animation"
                style="width: 210px;"
                @on-change="changeAnimation">
          <OptionGroup v-for="(item, index) in animations"
                       :key="index"
                       :label="item.name">
            <Option v-for="(itm, idx) in animations[index].children"
                    :key="itm.value"
                    :value="itm.value">{{itm.name}}</Option>
          </OptionGroup>
        </Select>
      </FormItem>
      <FormItem label="活动描述">
      ...
      <FormItem>
        <Tooltip :content="platform === 'mac' ? '按Command+Shift+S保存活动' : '按Control+Shift+S保存活动'"
                 placement="bottom-end">
          <Button type="primary"
                  :loading="isSaving"
                  icon="paper-airplane"
                  size="small"
                  :disabled="!activityInfoChanged"
                  @click="saveActivity">
  ...
  methods: {
    ...
    showPopup () {
      this.$store.commit(types.SHOW_FULL_SCREEN_POPUP, {
        ...
        subComType: 'preview',
        previewQRCode: 'http://10.2.5.0:8080/activity/preview?q=' + this.uuid,
