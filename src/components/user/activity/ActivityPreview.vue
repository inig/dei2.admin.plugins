<template>
  <div class="activity_preview_container">
    <div class="activity_preview_loading"
         v-if="pageLoadStatus === status.LOADING"
         :style="{width: isPc ? '375px' : '100%', height: isPc ? '667px' : '100%', 'box-shadow': isPc ? '0 0 20px 4px #ccc' : 'none'}">
      <div class="page_inner">
        <Spin size="large"></Spin>
      </div>
    </div>
    <div class="activity_preview_fail"
         v-else-if="pageLoadStatus === status.FAIL"
         :style="{width: isPc ? '375px' : '100%', height: isPc ? '667px' : '100%', 'box-shadow': isPc ? '0 0 20px 4px #ccc' : 'none'}">
      <div class="page_inner">
        数据加载错误
      </div>
    </div>
    <div class="activity_preview_wrapper"
         id="activity_preview_pages"
         v-else
         :style="{width: isPc ? '375px' : '100%', height: isPc ? '667px' : '100%', 'box-shadow': isPc ? '0 0 20px 4px #ccc' : 'none'}">
      <div class="activity_preview_page"
           v-for="(item, index) in activityInfo.pages"
           :key="item.uuid">
        <div class="page_inner"
             :style="[item.style]">
        </div>
