<template>
  <div class="upload-image-container" :style="{height: height + 'px' || '300px', width: width + 'px' || '100%'}">
    <Upload
        multiple
        type="drag"
        ...
        <div class="upload-plugin-area" :style="{height: height + 'px' || '300px', width: width + 'px' || '100%'}">
            ...
            <div class="edit-mask" :style="{height: height + 'px' || '300px', width: width + 'px' || '100%'}">
                <Icon :type="icon || 'edit'" size="24"></Icon>
            </div>
        </div>
    </Upload>
  </div>
...
<style>
  ...
  .upload-image-container .ivu-upload-drag {
      border: none;
      border-radius: 0;
      background-color: transparent;
  }
  .upload-image-container .ivu-upload-drag .edit-mask {
    opacity: 0;
    transition: opacity .3s ease-in-out;
  }
  .upload-image-container .ivu-upload-drag:hover {
      border: none;
  }
  .upload-image-container .ivu-upload-drag:hover .edit-mask {
     opacity: 1;
  }
  .edit-mask {
    display: none;
